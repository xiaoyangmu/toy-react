...
class ElementWrapper extends Component{
    ...
    [RENDER_TO_DOM](range){
        ...
        for(let name in this.props){
            if(name.match(/^on([\s\S]+)$/)){
                root.addEventListener(RegExp.$1.replace(/^[\s\S]/, c => c.toLocaleLowerCase()), this.props[name])
            }else{
                if(name === 'className'){
...
export function createElement(type, attributs, ...children){
    ...
    for(let attrs in attributs){
        e.setAttribute(attrs, attributs[attrs]);
    }
